	...
	return (
		<>
			<Div className={cn("mb-2 flex items-center")}>
				...
				<NavLink to="create" className={cn("ml-auto px-4 py-2 flex items-center justify-center gap-4 border-2 border-sky-500 text-sky-500 rounded-md cursor-pointer hover:scale-105 active:scale-95 transition-scale ease-in-out duration-150")}>
					<Span className={cn("icon")}>add</Span>
					<Span>{t("pages.settings.new")}</Span>
				</NavLink>
			</Div>
				...
				{binders && binders?.map((binder) => (
					<BinderCard key={binder.uuid}>
						...
						<Div className={cn("flex justify-end gap-2 -mx-4 px-2")}>
							<NavLink to={binder.uuid} className={cn("flex items-center justify-center gap-2 px-2 md:px-4 py-2 text-sm md:text-base bg-sky-500 text-sky-50 rounded-md cursor-pointer hover:scale-105 active:scale-95 transition-scale ease-in-out duration-150")}>
								<Span className={cn("icon")}>edit</Span>
								<Span>{t("pages.settings.edit")}</Span>
							</NavLink>
							<Button
								onClick={() => onDeleteBinder(binder.uuid)}
								className={cn("flex items-center justify-center gap-2 px-2 md:px-4 py-2 text-sm md:text-base border-2 border-red-500 text-red-500 rounded-md cursor-pointer hover:scale-105 active:scale-95 transition-scale ease-in-out duration-150")}>
								<Span className={cn("icon")}>delete</Span>
								<Span>{t("pages.settings.delete")}</Span>
							</Button>
						</Div>
					</BinderCard>
				))}
